...
// KUNCI PERBAIKAN: Hilangkan filter stok > 0 agar barang habis tetap terlihat
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['checkout'])) {
    ...
    if ($nominal_bayar < $total_bayar) {
        ...
        try {
            ...
            mysqli_commit($conn);
            // Tombol Cetak Nota Manual disisipkan dengan ukuran besar (800x700)
            $success = "Transaksi berhasil! Kembalian: Rp " . number_format($kembalian, 0, ',', '.') .
                " <a href='cetak_nota.php?id=$id_jual' target='CetakNota' onclick=\"window.open(this.href, 'CetakNota', 'width=800,height=700,left=200,top=100'); return false;\" class='ml-4 bg-green-700 hover:bg-green-800 text-white px-3 py-1 rounded text-sm font-bold transition-colors'>🖨️ Cetak Nota</a>";
            $id_nota_baru = $id_jual;

            // Refresh data katalog agar stok terbaru langsung muncul (tanpa filter > 0)
            $query_barang = mysqli_query($conn, "SELECT * FROM barang ORDER BY nama_barang ASC");
            $katalog_barang = [];
            while ($row = mysqli_fetch_assoc($query_barang)) {
                $katalog_barang[] = $row;
...
<body class="bg-gray-100 flex h-screen overflow-hidden">
    ...
    <aside class="w-20 lg:w-64 bg-blue-900 text-white flex flex-col h-full shadow-lg transition-all duration-300">
        ...
        <nav class="flex-1 p-2 lg:p-4 space-y-2">
            ...
            <a href="riwayat_transaksi.php" class="flex items-center p-3 hover:bg-blue-800 rounded-lg transition-colors justify-center lg:justify-start">
                <span class="text-xl lg:mr-3">📄</span>
        ...
        <div class="w-full lg:w-2/3 h-full flex flex-col p-4 bg-gray-50">
            ...
            <div class="flex-1 overflow-y-auto custom-scroll pr-2">
                <div class="grid grid-cols-2 md:grid-cols-3 gap-4" id="katalogGrid">
                    <?php
                    foreach ($katalog_barang as $brg):
                        // KUNCI PERBAIKAN: Logika Visual untuk Barang Habis
                        $is_empty = $brg['stok_aktual'] <= 0;
                        $card_class = $is_empty ? 'bg-gray-100 opacity-60 cursor-not-allowed' : 'bg-white hover:shadow-md cursor-pointer transition-shadow';
                        $click_action = $is_empty ? "alert('Maaf, stok barang ini sedang kosong!');" : "addToCart({$brg['id_barang']}, '" . addslashes($brg['nama_barang']) . "', {$brg['harga_jual']}, {$brg['stok_aktual']}, 'Pcs')";
                    ?>
                        <div class="border border-gray-200 rounded-lg p-4 shadow-sm item-card <?= $card_class; ?>"
                            onclick="<?= $click_action; ?>">
                            <div class="font-bold <?= $is_empty ? 'text-gray-500' : 'text-gray-800' ?> text-sm md:text-base mb-1 nama-barang"><?= $brg['nama_barang']; ?></div>
                            <div class="text-xs text-gray-500 mb-2 sku-barang">SKU: <?= $brg['kode_sku']; ?></div>
                            <div class="flex justify-between items-end mt-4">
                                <div class="<?= $is_empty ? 'text-gray-400' : 'text-blue-600' ?> font-bold">Rp <?= number_format($brg['harga_jual'], 0, ',', '.'); ?></div>
                                <div class="text-xs <?= $is_empty ? 'bg-red-100 text-red-600' : 'bg-gray-100 text-gray-600' ?> px-2 py-1 rounded font-semibold">
                                    Stok: <?= floatval($brg['stok_aktual']); ?>
                                </div>
                            </div>
                        </div>
                    <?php endforeach; ?>
    ...
    <script>
        ...
        function renderCart(hilangkanFokus = true) {
            ...
            let htmlContent = `
                <div class="text-center text-gray-400 mt-10" id="emptyCartMsg" style="display: ${cart.length === 0 ? 'block' : 'none'};">
                    <span class="text-4xl block mb-2">🛒</span>
                    Belum ada barang di keranjang
                </div>
            `;
            ...
            cart.forEach(item => {
                let qtyAman = isNaN(item.qty) || item.qty === '' ? 0 : parseFloat(item.qty);
                    ...
                    <div class="mb-3 p-3 bg-white border border-gray-200 rounded-lg shadow-sm">
                        ...
                        <div class="flex justify-between items-center mt-2">
                            <div class="flex items-center">
                                <input type="number" step="any" id="${inputId}" value="${isNaN(item.qty) ? '' : item.qty}" oninput="ubahQty(${item.id}, this.value, ${item.max})" 
    ...
    <?php if (isset($id_nota_baru)): ?>
        <script>
            // Pop-up otomatis dengan ukuran besar yang proporsional untuk kertas A4
            window.open('cetak_nota.php?id=<?= $id_nota_baru; ?>', 'CetakNota', 'width=800,height=700,left=200,top=100');
        </script>
    <?php endif; ?>
</body>
